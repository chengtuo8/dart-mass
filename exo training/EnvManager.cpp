	...
	:mNumEnvs(num_envs)
{
	...
	// ===== Exo caches init =====
	exo_cols = mEnvs[0]->GetExoTorques().rows(); // active dofs size
	mExoTorques.resize(mNumEnvs, exo_cols);
	mExoEnergy.resize(mNumEnvs);
	mExoAvgPower.resize(mNumEnvs);

	mCtrlEnergy.resize(mNumEnvs);
	mCtrlAvgPower.resize(mNumEnvs);
...
// ===================== Exo wrappers =====================
void
EnvManager::SetUseExo(bool use)
{
	for(int id=0; id<mNumEnvs; ++id)
		mEnvs[id]->SetUseExo(use);
}
bool
EnvManager::GetUseExo()
{
	return mEnvs[0]->GetUseExo();
}
void
EnvManager::SetExoTorqueLimits(const Eigen::VectorXd& limits)
{
	// one set of limits applied to all envs (active dofs size)
	for(int id=0; id<mNumEnvs; ++id)
		mEnvs[id]->SetExoTorqueLimits(limits);
}
void
EnvManager::SetExoTorques(const Eigen::MatrixXd& torques)
{
	// torques: [num_envs x exo_cols], each row for one env
	for(int id=0; id<mNumEnvs; ++id)
		mEnvs[id]->SetExoTorques(torques.row(id).transpose());
}
const Eigen::MatrixXd&
EnvManager::GetExoTorques()
{
#pragma omp parallel for
	for(int id=0; id<mNumEnvs; ++id)
		mExoTorques.row(id) = mEnvs[id]->GetExoTorques();
	return mExoTorques;
}
const Eigen::VectorXd&
EnvManager::GetEpisodeExoEnergyVec()
{
	for(int id=0; id<mNumEnvs; ++id)
		mExoEnergy[id] = mEnvs[id]->GetEpisodeExoEnergy();
	return mExoEnergy;
}
const Eigen::VectorXd&
EnvManager::GetEpisodeExoAvgPowerVec()
{
	for(int id=0; id<mNumEnvs; ++id)
		mExoAvgPower[id] = mEnvs[id]->GetEpisodeExoAvgPower();
	return mExoAvgPower;
}

const Eigen::VectorXd& EnvManager::GetEpisodeCtrlEnergyVec(){
  for(int id=0; id<mNumEnvs; ++id)
    mCtrlEnergy[id] = mEnvs[id]->GetEpisodeCtrlEnergy();
  return mCtrlEnergy;
}
const Eigen::VectorXd& EnvManager::GetEpisodeCtrlAvgPowerVec(){
  for(int id=0; id<mNumEnvs; ++id)
    mCtrlAvgPower[id] = mEnvs[id]->GetEpisodeCtrlAvgPower();
  return mCtrlAvgPower;
}
...
PYBIND11_MODULE(pymss, m)
{
	py::class_<EnvManager>(m, "pymss")
		...
		// ===== Exo to Python =====
		.def("SetUseExo",                 &EnvManager::SetUseExo)
		.def("GetUseExo",                 &EnvManager::GetUseExo)
		.def("SetExoTorqueLimits",        &EnvManager::SetExoTorqueLimits)
		.def("SetExoTorques",             &EnvManager::SetExoTorques)
		.def("GetExoTorques",             &EnvManager::GetExoTorques)
		.def("GetEpisodeExoEnergyVec",    &EnvManager::GetEpisodeExoEnergyVec)
		.def("GetEpisodeExoAvgPowerVec",  &EnvManager::GetEpisodeExoAvgPowerVec)

		.def("GetEpisodeCtrlEnergyVec",   &EnvManager::GetEpisodeCtrlEnergyVec)
		.def("GetEpisodeCtrlAvgPowerVec", &EnvManager::GetEpisodeCtrlAvgPowerVec);
}
